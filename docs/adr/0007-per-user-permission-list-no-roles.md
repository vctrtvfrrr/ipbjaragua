...
Definido que o painel terá múltiplos **Usuários** autenticados (ver [ADR-0006](./0006-panel-auth-google-oauth-invite-allowlist.md)), é preciso decidir como modelar _o que cada um pode fazer_. As entidades sob gestão são as sete do domínio (Boletim, Artigo, Liturgia, Aviso, Música, Membro, Agenda), mais a própria gestão de Usuários.
...
A autorização é uma **lista de Permissões por Usuário**, sem papéis. Uma **Permissão** tem a forma _entidade × ação_, com os quatro verbos CRUD **explícitos** (incluindo _read_): `read`, `create`, `update`, `delete` por entidade. Ver é uma Permissão própria — existe Usuário que só visualiza uma entidade sem poder alterá-la.
...
Tornar o _read_ explícito custa pouco (mais um verbo no catálogo) e habilita o caso real de um Usuário só-leitura, que um modelo com read implícito não expressaria.
