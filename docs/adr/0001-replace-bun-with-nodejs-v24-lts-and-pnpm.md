...
The project used Bun as both its runtime and package manager from the start. Bun
was adopted for its fast installs and its native SQLite API (`bun:sqlite`). After
the database migration to SQLite + Drizzle ORM (IPJI1), `bun:sqlite` became the
only Bun-exclusive dependency left in the project.

The Nuxt/Nitro ecosystem evolved toward first-class Node.js support, while Bun
support remained secondary — requiring special flags (`--bun`) for tools like
Vitest to work correctly.

## Decision

Migrate to **Node.js v24 LTS** as the runtime and **pnpm v10** as the package
manager.
