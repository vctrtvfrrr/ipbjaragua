...
> **Emenda (2026-08-19):** a validação deixa de ter workflow próprio. `validate.yml` e `deploy.yml` viram um único `.gitea/workflows/ci.yml` com três jobs — `validate` (em todo push, em qualquer branch) → `build` → `deploy` —, os dois últimos condicionados a `master` via `if: gitea.ref == 'refs/heads/master'`. O `concurrency: group: deploy` fica no job `deploy`, não no workflow: em nível de workflow serializaria a validação de todas as branches numa fila única que não cancela. Com build e deploy em jobs separados, **a invariante "build e deploy compartilham o daemon do host" deixa de valer** — nada garante que os dois jobs caiam no mesmo runner, e a `deploy-stack` não faz pull por conta própria. Por isso o `compose.yml` passa a declarar `pull_policy: always`, exatamente a mitigação que a seção de Consequências abaixo previa. O job `deploy` também precisa do seu próprio `actions/checkout`: a `deploy-stack` copia o `compose.yml` do workspace do repo chamador (input `compose-file`, default `./compose.yml`), então um workspace vazio falha o deploy.
