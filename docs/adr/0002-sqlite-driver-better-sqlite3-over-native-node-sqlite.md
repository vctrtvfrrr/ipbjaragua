...
With the migration from Bun to Node.js (IPJA1), `bun:sqlite` — Bun's native
SQLite API — had to be replaced. Node.js v22.5+ introduced an experimental native
SQLite module (`node:sqlite`). Drizzle ORM, already adopted in the project, would
...
Reassess `node:sqlite` once Drizzle ships stable support (conventional semantic
versioning). A future migration would remove `better-sqlite3` and
`@types/better-sqlite3` from the dependencies.
