...
- **Sessão = cookie httpOnly/secure/sameSite, assinado e stateless**, carregando apenas a identidade (id/e-mail do Usuário), com **expiração de 30 dias**. Sem tabela de sessão nem de token.
- **Revogação vale no próximo request:** a guarda autoritativa (layout do route group do admin) recarrega a linha `users` a cada acesso (confere existência e `status='active'`) e carrega as Permissões. Remover ou marcar a linha em `users` derruba o acesso no próximo request, mesmo com cookie ainda íntegro.
