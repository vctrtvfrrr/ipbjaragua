...
- `disabled` é terminal só para o **login** — um admin pode reativá-lo (`disabled→active`, Permissões preservadas); `disabled→pending` segue proibido. O Convite passa a **normalizar** o e-mail (`trim` + `lowercase`) na gravação, fechando a folga entre o `unique` (string crua) e o match case-insensitive do login. O `name` é backfilled do Google no login quando vazio (fill-if-empty, sem sobrescrever).
