...
describe('MeetingMinuteYearNav', () => {
  it('names the year each link leads to', () => {
    render(<MeetingMinuteYearNav book="mesa-administrativa" previousYear={2021} nextYear={2023} />)

    expect(screen.getByRole('link', { name: 'Atas de 2021' })).toHaveAttribute(
      'href',
      '/admin/meeting-minutes/mesa-administrativa?year=2021'
    )
    expect(screen.getByRole('link', { name: 'Atas de 2023' })).toHaveAttribute(
      'href',
      '/admin/meeting-minutes/mesa-administrativa?year=2023'
    )
  ...
  it('omits the direction that has no year left', () => {
    const { rerender } = render(<MeetingMinuteYearNav book="mesa-administrativa" previousYear={null} nextYear={2020} />)
    ...
    rerender(<MeetingMinuteYearNav book="mesa-administrativa" previousYear={2025} nextYear={null} />)
  ...
  it('renders nothing when there is a single year to show', () => {
    render(<MeetingMinuteYearNav book="mesa-administrativa" previousYear={null} nextYear={null} />)
